...
export const useAdminStore = defineStore('admin', () => {
    ...
    function decrementPendingOperators() {
        if (stats.value)
            stats.value.pending_operators = Math.max(
                0,
                stats.value.pending_operators - 1,
            );
    }

    function decrementPendingDrivers() {
        if (stats.value)
            stats.value.pending_drivers = Math.max(
                0,
                stats.value.pending_drivers - 1,
            );
    }
