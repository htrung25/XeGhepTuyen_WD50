...
<template>
  <div class="flex h-screen bg-slate-50 overflow-hidden">
    <!-- Sidebar -->
    <aside :class="[
      'flex flex-col bg-gray-900 transition-all duration-300 shrink-0',
      ...
      <div class="flex items-center gap-3 px-4 py-5 border-b border-gray-800">
        ...
        <span v-if="!sidebarCollapsed" class="text-white font-bold text-base truncate">
          XeGhep Admin
        </span>
      </div>
      ...
      <nav class="flex-1 py-4 overflow-y-auto">
        <template v-for="item in navItems" :key="item.path">
          ...
          >
            ...
            <!-- User-check icon -->
            <svg v-else-if="item.icon === 'user-check'" class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <!-- Map icon -->
            <svg v-else-if="item.icon === 'map'" class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" />
            </svg>
            <!-- Cash icon -->
            <svg v-else-if="item.icon === 'cash'" class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" />
            </svg>
            <!-- Tag icon -->
            <svg v-else-if="item.icon === 'tag'" class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
            </svg>
            <!-- Ticket icon -->
            <svg v-else-if="item.icon === 'ticket'" class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <div class="border-t border-gray-800 p-4">
        <button
          @click="handleLogout"
          :class="[
            'flex items-center gap-3 w-full px-2 py-2 rounded-lg text-gray-400 hover:text-white hover:bg-gray-800 text-sm transition-colors',
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      ...
      <header class="bg-white border-b border-slate-200 px-6 py-3.5 flex items-center justify-between shrink-0">
        <div class="flex items-center gap-3">
          ...
          <span class="text-sm font-medium text-gray-700">Hệ thống quản trị XeGhep</span>
        </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex items-center gap-2">
            ...
            <span class="text-sm font-medium text-gray-700 hidden md:block">{{ adminName }}</span>
          </div>
        </div>
      </header>
