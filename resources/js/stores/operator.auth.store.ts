...
export const useOperatorAuthStore = defineStore('operatorAuth', () => {
  ...
  const user     = ref<UserInfo | null>(JSON.parse(localStorage.getItem('operator_user') ?? 'null'))
  const operator = ref<OperatorInfo | null>(JSON.parse(localStorage.getItem('operator_info') ?? 'null'))
