...
describe('customerApi → Wayfinder route contract', () => {
  ...
  it('searchTrips resolves to the PUBLIC binding GET /api/public/trips', () => {
    customerApi.searchTrips({ from_city: 'Hà Nội', to_city: 'Hải Phòng', date: '2026-06-21', passengers: 2 })
    expect(apiClient.send).toHaveBeenCalledWith({
  ...
  it('getPublicTrip resolves to the PUBLIC binding GET /api/public/trips/{id}', () => {
    ...
    expect(apiClient.send).toHaveBeenCalledWith({ url: '/api/public/trips/trip-1', method: 'get' })
  })

  it('getTripSeats keeps the CUSTOMER (authed) binding for the seat map', () => {
    customerApi.getTripSeats('trip-1')
    expect(apiClient.send).toHaveBeenCalledWith({ url: '/api/customer/trips/trip-1/seats', method: 'get' })
  })
  ...
  it('createBooking resolves to POST /api/customer/bookings', () => {
    ...
    expect(apiClient.send).toHaveBeenCalledWith({ url: '/api/customer/bookings', method: 'post' }, payload)
  })
  ...
  it('getWallet resolves to GET /api/customer/wallet', () => {
    ...
    expect(apiClient.send).toHaveBeenCalledWith({ url: '/api/customer/wallet', method: 'get' })
  })

  it('markAllRead resolves to PUT /api/customer/notifications/read-all', () => {
    customerApi.markAllRead()
    expect(apiClient.send).toHaveBeenCalledWith({ url: '/api/customer/notifications/read-all', method: 'put' })
  })
  ...
  it('submitPartnerApplication uploads multipart via sendForm to the public route', () => {
    ...
    expect(apiClient.sendForm).toHaveBeenCalledWith({ url: '/api/public/partner-applications', method: 'post' }, form)
  })
})
