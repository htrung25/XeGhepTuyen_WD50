...
type ReqFn = (config: { headers: Record<string, string> }) => {
...
describe('apiClient envelope handling', () => {
    ...
    it('unwraps the Laravel { success, data } envelope exactly one level on GET', async () => {
        http.get.mockResolvedValueOnce({
            data: { success: true, data: { total_revenue: 999 } },
        });
    ...
    it('returns the server message on POST success alongside data', async () => {
        http.post.mockResolvedValueOnce({
            data: { success: true, data: { id: 'x' }, message: 'Đã duyệt' },
        });

        const res = await apiClient.post('/admin/operators/x/approve', {
            commission_rate: 10,
        });
...
describe('apiClient interceptors', () => {
    ...
    it('clears the token and redirects to the portal login on an expired-session 401', async () => {
        ...
        await expect(
            responseErrorHandlers[0]({
                response: { status: 401 },
                config: { url: '/api/driver/trips' },
            }),
        ).rejects.toBeDefined();
