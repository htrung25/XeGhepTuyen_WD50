...
describe('useAdminStore', () => {
  ...
  it('fetchDashboard maps data.stats into state and toggles isLoading', async () => {
    vi.mocked(adminApi.getDashboard).mockResolvedValue({ data: { stats }, message: null, error: null })
  ...
  it('fetchPendingOperators requests status=pending and fills the list', async () => {
    const operators = [{ id: 'o1', company_name: 'Bắc Hà', owner_name: 'A', status: 'pending', created_at: '' }]
    vi.mocked(adminApi.getOperators).mockResolvedValue({ data: operators, message: null, error: null })
    const store = useAdminStore()

    await store.fetchPendingOperators()

    expect(adminApi.getOperators).toHaveBeenCalledWith({ status: 'pending' })
  ...
  it('decrement helpers reduce the pending counters without going below zero', async () => {
    vi.mocked(adminApi.getDashboard).mockResolvedValue({ data: { stats: { ...stats, pending_operators: 1 } }, message: null, error: null })
    const store = useAdminStore()
