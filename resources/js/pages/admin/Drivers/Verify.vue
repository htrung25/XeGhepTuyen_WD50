...
const credResult = ref<{ phone: string } | null>(null);
const credTitle = ref('');
...
async function approveDriver(d: DriverDoc) {
    ...
    credTitle.value = 'Đã duyệt tài xế & gửi mật khẩu qua SMS';
...
async function resetDriverPassword(d: DriverDoc) {
    ...
    credTitle.value = 'Đã cấp lại mật khẩu & gửi qua SMS';
    credResult.value = data;
    credModal.value = true;
}
...
<template>
    <div>
        <div class="mb-6 flex items-center justify-between">
            ...
            <button @click="loadDrivers" class="text-sm font-medium text-red-600 hover:text-red-700">
                Làm mới
            </button>
        </div>
        ...
        <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
            ...
            <div class="relative min-w-[240px] flex-1 sm:max-w-xs">
                <svg class="absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2 text-gray-400" fill="none"
                    stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
                <input v-model="search" type="text" placeholder="Tìm theo tên tài xế..."
        ...
        <div v-if="isLoading" class="space-y-4">
            <div v-for="i in 3" :key="i" class="h-52 animate-pulse rounded-xl border border-slate-200 bg-white p-6" />
        </div>
        ...
        <div v-else class="space-y-5">
            <div v-for="d in filtered" :key="d.id" class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex gap-6">
                    <!-- Left: driver info -->
                    <div class="w-44 shrink-0">
                        <div class="mb-3 h-20 w-20 overflow-hidden rounded-xl bg-gray-100">
                            <img v-if="d.photo_url" :src="d.photo_url" :alt="d.full_name"
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="mb-4 flex gap-3">
                            <div class="group relative cursor-zoom-in" @click="
                                zoomedImage =
                                d.documents?.id_card_front ?? null
                                ">
                                <div class="h-24 w-36 overflow-hidden rounded-lg border border-gray-200 bg-gray-100">
                                    <img v-if="d.documents?.id_card_front" :src="d.documents.id_card_front"
                                        alt="CMND mặt trước" class="h-full w-full object-cover" />
                                    <div v-else
                            ...
                            <div class="group relative cursor-zoom-in" @click="
                                ...
                                ">
                                <div class="h-24 w-36 overflow-hidden rounded-lg border border-gray-200 bg-gray-100">
                                    <img v-if="d.documents?.id_card_back" :src="d.documents.id_card_back"
                                        alt="CMND mặt sau" class="h-full w-full object-cover" />
                                    <div v-else
                                        class="flex h-full w-full items-center justify-center p-2 text-center text-xs text-gray-400">
                                        Chưa có ảnh
                                    </div>
                        ...
                        <div class="flex items-start gap-4">
                            <div class="cursor-zoom-in" @click="
                                zoomedImage =
                                d.documents?.driver_license ?? null
                                ">
                                <div class="h-24 w-36 overflow-hidden rounded-lg border border-gray-200 bg-gray-100">
                                    <img v-if="d.documents?.driver_license" :src="d.documents.driver_license" alt="GPLX"
                                        class="h-full w-full object-cover" />
                                    <div v-else
                                        class="flex h-full w-full items-center justify-center p-2 text-center text-xs text-gray-400">
                                        Chưa có ảnh
                    ...
                    <!-- Right: action panel -->
                    <div v-if="d.status === 'pending'" class="flex w-44 shrink-0 flex-col gap-2">
                        <button v-if="can('drivers.review')" @click="approveDriver(d)"
                            class="w-full rounded-lg bg-green-600 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-green-700">
                            Duyệt tài xế
                        </button>
                        <button v-if="can('drivers.review')" @click="openReject(d)"
                    ...
                    <div v-else-if="d.status === 'verified'" class="flex w-44 shrink-0 flex-col gap-2">
                        <button v-if="can('drivers.reset_password')" @click="resetDriverPassword(d)"
                            class="w-full rounded-lg bg-slate-100 px-4 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-200">
                            Cấp lại mật khẩu
                        </button>
                        <button v-if="can('drivers.suspend')" @click="openReject(d)"
    ...
    <Teleport to="body">
        <div v-if="zoomedImage" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80"
            ...
            <img :src="zoomedImage" class="max-h-[80vh] max-w-2xl rounded-xl object-contain" />
        </div>
    </Teleport>

    <!-- Credentials modal (sau duyệt / cấp lại mật khẩu) -->
    <Teleport to="body">
        <div v-if="credModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
                ...
                <p class="mb-4 text-sm text-gray-500">
                    Mật khẩu đăng nhập đã được gửi cho tài xế qua SMS. Để bảo đảm
                    quyền lợi tài xế, admin không xem được mật khẩu.
                </p>

                <div v-if="credResult" class="mb-4 space-y-2 rounded-xl bg-gray-50 p-4 text-sm">
                ...
                <p class="mb-4 text-xs text-gray-500">
                    Nếu tài xế không nhận được SMS, hãy dùng "Cấp lại mật khẩu" —
                    hệ thống sẽ gửi mật khẩu mới qua SMS.
                </p>
    ...
    <Teleport to="body">
        <div v-if="showRejectModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
                ...
                <div class="mb-5">
                    <label class="mb-1.5 block text-sm font-medium text-gray-700">Lý do <span
