    ...
    <div class="mx-auto max-w-7xl p-6">
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <p class="mt-0.5 text-sm text-gray-500">
                    Quản lý tài khoản và vai trò nhân viên quản trị.
                </p>
            </div>
            <button
                v-if="canManage"
        ...
        >
            ...
            <div class="flex gap-1 rounded-lg bg-gray-100 p-1">
                <button
                    ...
                    :key="s.v"
                    @click="
                        statusFilter = s.v as typeof statusFilter;
                        fetchStaff();
                    "
                    :class="[
        ...
        <div v-if="loading" class="space-y-3">
            <div
                v-for="i in 4"
                :key="i"
                class="h-14 animate-pulse rounded-xl border border-slate-200 bg-white"
            />
        </div>
        <div
            v-else-if="errorMsg"
        ...
        >
            <table class="w-full text-left text-sm">
                <thead>
                    <tr
                        class="border-b border-slate-200 bg-slate-50 text-[11px] font-bold tracking-wide text-slate-500 uppercase"
                    >
                        <th class="px-5 py-3">Nhân viên</th>
                        <th class="px-5 py-3">Vai trò</th>
                        <th class="px-5 py-3">Trạng thái</th>
                        <th class="px-5 py-3">Đăng nhập gần nhất</th>
                        <th v-if="canManage" class="px-5 py-3 text-right">
                            Hành động
                        </th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 text-slate-600">
                    ...
                    >
                        ...
                        <td v-if="canManage" class="px-5 py-3">
                            <div
                                class="flex justify-end gap-2 text-xs font-semibold"
                            >
                                <button
                                    @click="openEdit(s)"
                                ...
                                <button
                                    ...
                                    class="rounded-lg border border-blue-200 px-3 py-1.5 text-blue-600 hover:bg-blue-50"
                                >
                                    Đặt lại MK
                                </button>
                                <button
            ...
            >
                <div class="w-full max-w-lg rounded-2xl bg-white shadow-2xl">
                    ...
                    <div class="space-y-4 p-6">
                        ...
                        <p
                            v-if="!editing"
                            class="rounded-lg bg-blue-50 p-3 text-xs text-blue-700"
                        >
                            Hệ thống sẽ tạo mật khẩu tạm và hiển thị 1 lần sau
                            khi tạo.
                        </p>
                    </div>
                    <div
                        class="flex justify-end gap-3 border-t border-slate-100 p-4"
                    >
                        <button
                            @click="showModal = false"
            ...
            >
                ...
                >
                    ...
                    <p class="mb-3 text-xs text-slate-500">
                        {{ credLabel }}. Hãy sao chép và bàn giao — mật khẩu chỉ
                        hiển thị một lần.
                    </p>
                    <div
                        class="mb-4 flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 font-mono text-lg font-bold tracking-wider text-slate-800"
                    >
                        {{ credPassword }}
                    </div>
                    <button
