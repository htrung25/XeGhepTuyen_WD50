...
const error      = ref('')
const search     = ref('')
const statusTab  = ref<'all' | 'pending_payment' | 'confirmed' | 'cancelled' | 'completed'>('all')
const page       = ref(1)
...
const statusConfig: Record<string, { label: string; cls: string }> = {
  pending_payment: { label: 'Chờ thanh toán', cls: 'bg-yellow-50 text-yellow-700' },
  ...
  <div class="p-6 max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">{{ totalCount }} đặt chỗ</p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 mb-5 space-y-3">
      ...
      <div class="flex gap-3 items-center">
        <div class="relative flex-1 max-w-sm">
          ...
          <input v-model="search" type="text" placeholder="Tìm mã đặt, tên, SĐT khách..."
            class="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500"
    ...
    <div v-else-if="error" class="bg-white rounded-xl border border-gray-200 p-12 text-center">
      ...
      <button @click="fetchBookings" class="px-4 py-2 bg-amber-500 text-white text-sm rounded-lg">Thử lại</button>
    </div>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-gray-50 border-b border-gray-200">
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Mã đặt</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Khách hàng</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Chuyến đi</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Ghế / Điểm</th>
              ...
              <th class="px-4 py-3 text-center text-xs font-semibold text-gray-500 uppercase">Chi tiết</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            <tr v-if="bookings.length === 0">
              <td colspan="8" class="px-4 py-12 text-center text-gray-400 text-sm">Không có đặt chỗ nào</td>
            </tr>
            <tr v-for="b in bookings" :key="b.id" class="hover:bg-gray-50 transition-colors">
              <!-- Code -->
              <td class="px-4 py-3">
                <span class="font-mono text-xs font-semibold text-gray-700">{{ b.tracking_code }}</span>
              </td>
              <!-- Passenger -->
              <td class="px-4 py-3">
                <p class="font-medium text-gray-900">{{ b.passenger_name }}</p>
                <p class="text-xs text-gray-400">{{ b.passenger_phone }}</p>
              </td>
              <!-- Trip -->
              <td class="px-4 py-3">
                ...
                <p class="text-xs text-gray-400">{{ b.trip?.depart_at ? fmtDateTime(b.trip.depart_at) : '—' }}</p>
              </td>
              <!-- Seats / Stops -->
              <td class="px-4 py-3">
                ...
                <p class="text-xs text-gray-400 mt-0.5">
                  {{ b.pickup_stop?.stop_name }} → {{ b.dropoff_stop?.stop_name }}
                </p>
              </td>
              <!-- Amount -->
              ...
              <!-- Date -->
              <td class="px-4 py-3 text-gray-500 text-sm">{{ fmtDate(b.created_at) }}</td>
              <!-- Detail -->
              <td class="px-4 py-3 text-center">
                <button @click="detailItem = b; detailModal = true"
                  class="px-2.5 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-700 text-xs font-medium rounded-lg transition-colors">
                  Xem
                </button>
              </td>
            </tr>
    ...
    <!-- Detail modal -->
    <Teleport to="body">
      <div v-if="detailModal && detailItem" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-black/40" @click="detailModal = false" />
        <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg p-6">
          <div class="flex items-center justify-between mb-5">
            <h3 class="text-lg font-bold text-gray-900">Chi tiết đặt chỗ</h3>
          ...
          <div class="space-y-3 text-sm">
            <div class="flex justify-between">
              ...
              <span class="font-mono font-semibold">{{ detailItem.tracking_code }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">Khách hàng</span>
              <span class="font-medium">{{ detailItem.passenger_name }} — {{ detailItem.passenger_phone }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">Tuyến</span>
              <span>{{ detailItem.trip?.route?.origin_city }} → {{ detailItem.trip?.route?.dest_city }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">Giờ khởi hành</span>
              <span>{{ detailItem.trip?.depart_at ? fmtDateTime(detailItem.trip.depart_at) : '—' }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">Ghế</span>
              <span class="font-mono">{{ detailItem.seat_codes?.join(', ') }}</span>
            </div>
            <div class="flex justify-between">
            ...
            <div class="flex justify-between">
              ...
              <span>{{ detailItem.dropoff_stop?.stop_name }}</span>
            </div>
            <div class="flex justify-between border-t pt-3 mt-3">
              <span class="text-gray-500">Tổng tiền</span>
              <span class="text-lg font-bold text-amber-600">{{ fmtMoney(detailItem.total_amount) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">Trạng thái</span>
              <span :class="['inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold',
                statusConfig[detailItem.status]?.cls ?? 'bg-gray-100 text-gray-600']">
