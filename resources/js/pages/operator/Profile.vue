...
<template>
    <div class="mx-auto max-w-5xl p-6">
        ...
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <p class="mt-0.5 text-sm text-slate-500">
                    Cập nhật thông tin công ty, chi tiết ngân hàng và quản lý
                    tài khoản.
                </p>
            </div>
            <button
        ...
        <div
            v-if="isLoading"
            class="grid grid-cols-1 gap-6 md:grid-cols-[320px_1fr]"
        >
            <div class="space-y-6">
                <div
                    class="h-72 animate-pulse rounded-xl border border-slate-200 bg-white"
                />
                <div
            ...
            <div class="space-y-6">
                <div
                    class="h-[480px] animate-pulse rounded-xl border border-slate-200 bg-white"
                />
                <div
                    class="h-80 animate-pulse rounded-xl border border-slate-200 bg-white"
                />
            </div>
        </div>
        ...
        <div
            v-else
            class="grid grid-cols-1 items-start gap-6 md:grid-cols-[320px_1fr]"
        >
            <!-- LEFT COLUMN -->
            <div class="space-y-6">
                ...
                >
                    ...
                    <div class="group relative mx-auto mb-4 h-28 w-28">
                        <div
                            class="flex h-28 w-28 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-slate-50"
                        >
                            <!-- Preview uploaded image -->
                            <img
                                v-if="
                                    logoPreview ||
                                    operatorData?.operator?.logo_url
                                "
                                :src="
                                    logoPreview ||
                                    operatorData?.operator?.logo_url
                            ...
                            <div v-else class="p-2 text-center">
                                <span
                                    class="text-3xl font-black text-amber-500"
                                >
                        ...
                        <button
                            ...
                            class="absolute -right-2 -bottom-2 flex h-8 w-8 items-center justify-center rounded-lg bg-amber-500 text-white shadow-md transition-colors hover:bg-amber-600"
                            title="Tải logo lên"
                        >
                            <svg
                                class="h-4 w-4"
                    ...
                    <div class="mt-5 space-y-3 border-t border-slate-100 pt-4">
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-slate-400"
                                >Chiết khấu hệ thống</span
                            >
                            <span
                                class="rounded border border-amber-200 bg-amber-50 px-2 py-0.5 font-bold text-amber-700 text-slate-700"
                            >
                                {{
                                    operatorData?.operator?.commission_rate ??
                                    0
                                }}%
                            </span>
                        </div>
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-slate-400"
                                >Điện thoại liên hệ</span
                            >
                            <span
                                class="font-mono font-medium text-slate-700"
                                >{{ operatorData?.phone }}</span
                            >
                        </div>
                    </div>
                </div>
                ...
                >
                    ...
                    <p class="mb-4 text-xs text-slate-400">
                        Các tài liệu pháp lý này chỉ được sửa đổi bởi Quản trị
                        viên hệ thống.
                    </p>
                    ...
                    <div class="space-y-3 text-xs">
                        <div>
                            ...
                            >
                            <span
                                class="block rounded border border-slate-100 bg-slate-50 p-2 font-mono font-semibold text-slate-800"
                            >
                                {{
                                    operatorData?.operator?.tax_code ||
                                    'Chưa cập nhật'
                                }}
                            </span>
                        </div>
                        <div>
                            ...
                            >
                                {{
                                    operatorData?.operator?.business_license ||
                                    'Chưa cập nhật'
                                }}
                            </span>
                        </div>
            ...
            <div class="space-y-6">
                ...
                >
                    <div>
                        ...
                        <p class="mt-0.5 text-xs text-slate-400">
                            Cập nhật thông tin định danh và tài khoản giao dịch
                            ngân hàng để thực hiện thanh toán.
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div class="space-y-1.5 sm:col-span-2">
                            <label
                                class="block text-sm font-medium text-slate-700"
                                >Tên nhà xe / Tên doanh nghiệp</label
                            >
                            <input
                        ...
                        <div class="space-y-1.5 sm:col-span-2">
                            <label
                                class="block text-sm font-medium text-slate-700"
                                >Mô tả giới thiệu nhà xe</label
                            >
                            <textarea
                                v-model="form.description"
                                rows="3"
                                placeholder="Viết giới thiệu ngắn về chất lượng dịch vụ của nhà xe..."
                                class="w-full resize-none rounded-lg border border-slate-200 p-3.5 text-sm transition-all focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none"
                            />
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input
                                ...
                                class="h-10 w-full rounded-lg border border-slate-200 px-3.5 font-mono text-sm transition-all focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none"
                            />
                        </div>
                        ...
                        <div class="space-y-1.5 sm:col-span-2">
                            <label
                                class="block text-sm font-medium text-slate-700"
                                >Tên chủ tài khoản ngân hàng</label
                            >
                            <input
                                v-model="form.bank_account_name"
                                type="text"
                                placeholder="Ví dụ: NGUYEN VAN A"
                                class="h-10 w-full rounded-lg border border-slate-200 px-3.5 text-sm uppercase transition-all focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none"
                            />
                        </div>
                    </div>
                    ...
                    <div
                        v-if="saveError"
                        class="flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 p-3.5 text-sm text-red-600"
                    >
                    ...
                    <!-- Submit -->
                    <div
                        class="flex items-center gap-3 border-t border-slate-100 pt-2"
                    >
                        <button
                            type="submit"
                            :disabled="saveLoading"
                            class="flex items-center gap-2 rounded-lg bg-amber-500 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-amber-600 disabled:opacity-60"
                        >
                            <div
                                v-if="saveLoading"
                ...
                >
                    <div>
                        ...
                        <p class="mt-0.5 text-xs text-slate-400">
                            Cập nhật mật khẩu để bảo vệ an toàn cho tài khoản
                            nhà xe của bạn.
                        </p>
                    </div>
                    ...
                    >
                        <button
                            type="submit"
                            :disabled="pwLoading"
                            class="flex items-center gap-2 rounded-lg bg-slate-800 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-slate-900 disabled:opacity-60"
                        >
                            <div
                                v-if="pwLoading"
                                class="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent"
