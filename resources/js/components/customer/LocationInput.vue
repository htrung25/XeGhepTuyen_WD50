...
function openMapModal() {
    ...
    currentMapAddress.value = props.modelValue || 'Đang lấy vị trí tâm bản đồ...';
...
<template>
    <div class="relative space-y-1.5 w-full">
        ...
        <div class="flex gap-2">
            ...
            <div class="relative flex-1">
                ...
                <input
                    ...
                    class="h-12 w-full rounded-xl border border-slate-200 bg-white pl-11 pr-4 text-base text-slate-900 placeholder-slate-400 focus:border-green-500 focus:ring-2 focus:ring-green-500/20 focus:outline-none transition-all duration-150"
                ...
                <div
                    ...
                    class="absolute z-30 mt-1.5 w-full rounded-xl border border-slate-200 bg-white shadow-xl max-h-60 overflow-y-auto"
                >
                    <button
                        v-for="item in suggestions"
                        :key="item.id"
                        type="button"
                        class="w-full px-4 py-3 text-left text-sm hover:bg-slate-50 transition border-b border-slate-100 last:border-0 flex items-start gap-2.5"
                        @mousedown="selectSuggestion(item)"
                    >
                        <span class="mt-0.5 text-slate-400 shrink-0">📍</span>
                        <div>
                            <p class="font-medium text-slate-800 text-[13.5px]">{{ item.text }}</p>
                            <p class="text-xs text-slate-500 mt-0.5 leading-relaxed">{{ item.place_name }}</p>
                        </div>
                    </button>
                </div>
            </div>
            ...
            <button
                type="button"
                class="h-12 px-4 rounded-xl border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 font-semibold text-sm transition flex items-center gap-1.5 shrink-0"
                @click="openMapModal"
            >
        ...
        >
            <div class="relative w-full max-w-2xl h-[85vh] bg-white rounded-2xl overflow-hidden shadow-2xl flex flex-col border border-slate-200">
                <!-- Modal Header -->
                <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                    <div>
                        <h3 class="font-bold text-slate-900 text-base">Chọn vị trí trên bản đồ</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Kéo thả bản đồ để ghim đúng vị trí đón/trả</p>
                    </div>
                    <button
                        type="button"
                ...
                <div class="flex-1 relative bg-slate-100">
                    ...
                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[calc(100%-8px)] pointer-events-none z-10 flex flex-col items-center">
                        <div class="bg-slate-900 text-white text-[11px] font-bold px-2 py-0.5 rounded shadow mb-1.5 transition-opacity" :class="isResolvingAddress ? 'opacity-50' : 'opacity-100'">
                            {{ isResolvingAddress ? 'Đang xác vị trí...' : 'Điểm tại đây' }}
                        </div>
                        <svg class="h-10 w-10 text-red-600 drop-shadow-lg" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" />
                        </svg>
                        <!-- Pin pulse shadow -->
                        <div class="h-2 w-2 bg-red-600/30 rounded-full animate-ping mt-0.5" />
                    </div>
                </div>
                ...
                <div class="p-6 border-t border-slate-100 bg-white space-y-4">
                    <div class="flex items-start gap-2.5">
                        <span class="mt-0.5 text-green-600 text-base">📍</span>
                        <div class="flex-1 min-w-0">
                            <p class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Địa chỉ hiện tại</p>
                            <p class="text-sm font-semibold text-slate-800 mt-0.5 leading-relaxed truncate" :class="isResolvingAddress ? 'text-slate-400' : ''">
                                {{ currentMapAddress }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-3">
                        <button
                            type="button"
                            class="flex-1 h-11 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl text-sm transition"
                            @click="closeMapModal"
                        >
                            Hủy
                        </button>
                        <button
                            ...
                            class="flex-1 h-11 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-xl text-sm shadow transition disabled:opacity-50"
