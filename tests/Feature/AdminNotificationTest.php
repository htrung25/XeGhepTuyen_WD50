...
it('badge sidebar: pending-counts đếm đơn đối tác chờ duyệt (việc thực tế)', function () {
    ...
    expect($data['/admin/operators'] ?? 0)->toBe(1);
});

it('pending-counts chỉ trả mục admin có quyền xem', function () {
    $role = AdminRole::create([
    ...
    $data = $this->getJson('/api/admin/pending-counts')->assertOk()->json('data');
    expect($data)->toHaveKey('/admin/drivers');
    expect($data)->not->toHaveKey('/admin/operators');
    expect($data)->not->toHaveKey('/admin/finance');
});
