...
const user = {
    id: 'u1',
    full_name: 'Chủ nhà xe',
    email: 'op@xeghep.vn',
    phone: '0912345678',
};
const operator = {
    id: 'op-1',
    company_name: 'Xe Ghép Bắc Hà',
    status: 'verified',
    commission_rate: 10,
};
...
describe('useOperatorAuthStore', () => {
    ...
    it('setAuth persists token, user and operator info', () => {
        ...
        expect(JSON.parse(localStorage.getItem('operator_user')!)).toEqual(
            user,
        );
        expect(JSON.parse(localStorage.getItem('operator_info')!)).toEqual(
            operator,
        );
    });
